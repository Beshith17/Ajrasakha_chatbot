...
function fallbackAnswer(input: AnswerInput) {
...
export async function generateAgricultureAnswer(input: AnswerInput) {
...
export async function analyzeImageWithVision(input: ImageDiagnosisInput): Promise<VisionDiagnosisResult | null> {
  if (!env.openAiApiKey) {
    return null;
  }
      ...
      messages: [
        {
          ...
          content: [
            {
              type: 'text',
              text: `Analyze this crop image for disease or pest issues. ${
                cropContext ? `Known crop hints: ${cropContext}.` : ''
...
If the crop appears healthy, say "Healthy crop".`,
  ...
  const data = await safeJson<{ choices?: Array<{ message?: { content?: string } }> }>(response);
  const modelText = data?.choices?.[0]?.message?.content ?? '';
  const parsed = modelText ? parseJsonBlock<VisionDiagnosisResult>(modelText) : null;
...
export async function explainDiagnosis(details: {
  ...
  const explanation = `${details.crop}: ${details.disease}. Symptoms include ${details.symptoms.join(
    ', ',
  )}. Recommended actions: ${details.recommendations.join(', ')}.`;
