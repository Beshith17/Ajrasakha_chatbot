...
function fallbackDiagnosis(input, overrides) {
    return {
...
function normalizeSeverity(probability) {
    if (probability >= 0.85) {
        return 'high';
    }
    if (probability >= 0.6) {
...
function needsVisionFallback(result) {
    const normalizedDiseaseName = result.diseaseName.trim().toLowerCase();
    return (result.diseaseName === 'Unidentified disease' ||
...
async function assessHealth(base64Image) {
...
function pickBestDiseaseSuggestion(suggestions = []) {
...
export async function analyzeCropImage(input) {
    ...
    if (visionFirstResult) {
        ...
        return {
            ...
            explanation: visionFirstResult.explanation ??
