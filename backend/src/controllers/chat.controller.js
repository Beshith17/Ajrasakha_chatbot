...
export async function createChatSession(req, res) {
    const { sessionName, farmerName, language, ownerUid, ownerEmail, ownerPhone } = req.body;
    if (!sessionName?.trim()) {
...
export async function joinChatSession(req, res) {
    const { sessionCode, ownerUid, ownerEmail, ownerPhone, farmerName } = req.body;
    if (!sessionCode?.trim()) {
...
export async function createFarmerMessage(req, res) {
    const { question, language, farmerName, sessionId, sessionCode, ownerUid, ownerEmail, ownerPhone } = req.body;
    if (!question?.trim()) {
...
export async function createExpertReply(req, res) {
    const { answer, language, expertName } = req.body;
    if (!answer?.trim()) {
...
export async function endChatSession(req, res) {
    ...
    emitSessionNotification('farmer', {
        sessionId: session.id,
        sessionCode: session.sessionCode,
        title: session.sessionName,
...
export async function clearChatSession(req, res) {
    const session = await ChatSession.findById(req.params.sessionId);
    if (!session) {
        return res.status(404).json({ message: 'Session not found.' });
    }
