...
function buildUserPayload(user: { _id: unknown; email?: string; phone?: string; displayName?: string | null }) {
  return {
    uid: String(user._id),
    email: user.email,
...
export async function signup(req: Request, res: Response) {
  ...
  const existingUser = await UserModel.findOne(
    identifierType === 'email' ? { email: normalizedIdentifier } : { phone: normalizedIdentifier },
  );
