...
export function ExpertDashboardPage() {
    ...
    return (<div className="workspace-grid expert-layout">
      <section className="glass-card fade-in">
        ...
        <form className="stack-form expert-reply-form" onSubmit={handleCreateSession}>
          <div className="responsive-grid">
            <label>
              Session name
              <input onChange={(event) => setNewSessionName(event.target.value)} placeholder="Tomato Advisory Batch" value={newSessionName}/>
            </label>
            <label>
              Farmer name
        ...
        <div className="history-list">
          {sessions.map((session) => (<article className={`history-card ${session._id === selectedSession?._id ? 'active' : ''}`} key={session._id}>
        ...
        {selectedSession ? (<>
            <div className="message-feed expert-feed">
              {selectedSession.messages
                ...
                .map((message) => (<article className={`message-bubble ${message.role === 'farmer' ? 'user' : 'assistant'} slide-up`} key={message.messageId}>
            ...
            <form className="stack-form expert-reply-form" onSubmit={handleSubmit}>
              <div className="session-action-row">
                <button className="delete-chat-button" disabled={selectedSession.messages.length === 0 || clearing} onClick={() => void handleClearChat()} type="button">
                  {clearing ? 'Clearing...' : 'Clear Chat'}
                </button>
                <button className="secondary-button" disabled={selectedSession.status === 'ended'} onClick={() => void handleEndSession()} type="button">
                  {selectedSession.status === 'ended' ? 'Session Ended' : 'End Session'}
                </button>
              </div>
              <label>
                Expert answer
                <textarea disabled={selectedSession.status === 'ended'} onChange={(event) => setAnswer(event.target.value)} placeholder={selectedSession.status === 'ended' ? 'This session is closed.' : 'Write the response the farmer should receive.'} rows={5} value={answer}/>
              </label>
