...
import type { ChatMessage, ChatSessionSummary, LanguageOption } from '../types';
...
export function FarmerDashboardPage() {
  ...
  async function handleSessionOpen(nextSessionId: string) {
    try {
  ...
  async function handleListenToReply(message: ChatMessage, key: string) {
    try {
        ...
        <form className="stack-form farmer-chat-composer" onSubmit={handleSubmit}>
          ...
          <label>
            ...
            <textarea
              ...
              placeholder="Describe the crop issue, stage, and symptoms."
              rows={4}
              value={question}
            />
          </label>
          ...
          {activeSession?.kind === 'chat' ? (
            <>
              ...
              <button
                ...
                disabled={!sessionId || messages.length === 0 || clearing}
            ...
            >
              ...
              {message.role !== 'farmer' ? (
                <button
                  ...
                  onClick={() => void handleListenToReply(message, message.messageId)}
                  type="button"
                >
      ...
      <section className="glass-card delay-in">
        ...
        <div className="history-section">
          ...
          <div className="history-list">
            {aiChats.map((session) => (
              <article className={`history-card ${session._id === sessionId ? 'active' : ''}`} key={session._id}>
